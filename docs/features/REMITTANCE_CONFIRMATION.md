...
若希望「只有匯款截圖」的訊息也能觸發匯款流程，請在 `.env` 中設定：
...
啟用後流程會：
1. 下載使用者傳來的圖片
2. 呼叫 Google Vision OCR 取得文字
3. 只有當 OCR 結果包含匯款關鍵字時才會觸發匯款通知

> 若未啟用 OCR 或辨識結果為空，就算使用者只傳圖片也不會回覆匯款訊息。

### 5. 語意過濾（避免誤判）

為了避免「明天處理匯款」「請問匯款方式？」等非完成訊息被誤判，可設定以下環境變數：
...
系統偵測到上述語意時，不會觸發匯款流程，而是回覆 `REMITTANCE_DEFER_REPLY` 內容，並把紀錄寫入 `src/data/remittance-intent-log.json` 供客服追蹤。
...
> 2025-11-21 更新：圖片會先由 OCR 辨識，僅當辨識出的文字包含匯款關鍵字時才會觸發匯款流程。

建議操作：
1. 先傳送含有匯款關鍵字的文字（如「我已匯款 1500 元」）
2. 或啟用 OCR 後，直接傳匯款截圖讓系統自動辨識

**範例**：
```
✅ 文字："已匯款" + 截圖
✅ 文字："轉帳完成" + 截圖
✅ （啟用 OCR）只傳截圖，但辨識內容包含「轉帳」
❌ 只有截圖且 OCR 無法辨識關鍵字
❌ 文字提到「明天匯款」「還沒匯」或使用疑問句（語意過濾會延後處理）
```
...
src/data/remittance-intent-log.json   # 語意過濾紀錄
```
...
[
  {
    "id": "intent_1732179999999_xxx123",
    "userId": "U1234567890abcdef",
    "messageText": "明天處理匯款謝謝",
    "reason": "postpone",
    "flags": {
      "hasPostpone": true,
      "hasNegative": false,
    ...
    "createdAt": "2025-11-21T05:50:00.000Z"
  }
]
```
